...
def modify_rendezvous_config():
    print("[INFO] Modifying Rendezvous task configuration for evaluation...")
    new_content = []
    eval_racing_cfg_path = os.path.normpath(os.path.join(os.getcwd(), "source", "isaaclab_tasks", "isaaclab_tasks", "rans", "tasks_cfg", "rendezvous_cfg.py"))
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            if "MultiTask" in args_cli.task:
                ...
                for i in range(num_tasks):
                    if not torch.all(task_completion_counts[i] >= args_cli.runs_per_env + 1).item():
                ...
                if torch.all(torch.sum(torch.cat(data["dones"], dim=-1).view(-1, env_cfg.scene.num_envs), dim=0) >= args_cli.runs_per_env + 1).item():
