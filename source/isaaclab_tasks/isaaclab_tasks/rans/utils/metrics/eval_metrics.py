...
class EvalMetrics:
    ...
    def calculate_metrics(self, data: dict)->None:
        ...
        reset_index, reset_env =  torch.where(self.data["dones"] > 0)
        cutoff_indices = torch.zeros((data['dones'].shape[-1], self.num_runs_per_env), dtype=torch.long, device=self.device)
            
        for i, reset_idx in enumerate(reset_index):
            env_idx = reset_env[i].item()
            if torch.any(cutoff_indices[env_idx] == 0):
                idx = torch.where(cutoff_indices[env_idx] == 0)[0][0].item()
                if cutoff_indices[env_idx, idx] == 0:
                    cutoff_indices[env_idx, idx] = reset_idx
        ...
        # Save Trajectories
        # Creates a panda DataFrame where each row is a time step of a trajectory
        self.save_extracted_trajectories_to_csv(max_workers=32)
    ...
    def save_extracted_trajectories_to_csv(self, max_workers=32):
        ...
        if not hasattr(self, 'extracted_trajectories') or not self.extracted_trajectories:
            print("[WARNING] No trajectories found to save.")
            return
        ...
        def _process_one(traj_idx):
            ...
            for key in keys:
                ...
                if arr.ndim == 1:
                    ...
                    data_cols[key] = arr.reshape(actual_len, -1)
