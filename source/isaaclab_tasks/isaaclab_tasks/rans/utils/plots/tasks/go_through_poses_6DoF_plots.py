...
class GoThroughPoses3DPlots(BaseTaskPlots, Registerable):
    def __init__(self, dfs: dict, trajectories_dfs: dict, labels: dict, env_info:dict, folder_path:list, plot_cfg:dict) -> None:
        ...
        self.task_name = "go_through_poses_3d"

        keys_set = set()
        for group_dfs in dfs.values():
            for df in group_dfs:
                keys_set.update(
                    key for key in df.columns if "success_rate" in key
                )
                keys_set.update(
                    key for key in df.columns if "orientation_error" in key
                )
