...
from isaaclab_tasks.rans import VisualGoThroughPoses3DCfg
...
class VisualGoThroughPoses3DTask(TaskCore):
    """
    ...
    def __init__(
        ...
        task_cfg: VisualGoThroughPoses3DCfg = VisualGoThroughPoses3DCfg(),
        task_uid: int = 0,
        num_envs: int = 1,
        device: str = "cuda",
    ...
    def set_goals(self, env_ids: torch.Tensor) -> None:
        ...
        - First goal: Random position inside the ISS box (constrained AABB).
        - Subsequent goals: Offset from previous goal using sampled spherical coordinates, then clamped to the AABB.
        ...
        Returns:
            None
        """
        # Box parameters (mirror posebox defaults if cfg not present)
        wall_width = getattr(self._task_cfg, "iss_box_wall_width", 2.0)
        wall_length = getattr(self._task_cfg, "iss_box_wall_length", 6.4)
        wall_height = getattr(self._task_cfg, "iss_box_wall_height", 2.0)
        x_shift = getattr(self._task_cfg, "iss_box_x_shift", -0.05)
        height_from_floor = getattr(self._task_cfg, "iss_box_height_from_floor", 0.3)
        # Use thickness as margin to keep away from the walls a bit
        margin = 0.3 #getattr(self._task_cfg, "iss_box_wall_thickness", 0.05)

        # Compute per-env AABB in world frame: [min_xyz, max_xyz]
        env_o = self._env_origins[env_ids]  # [Ne, 3]
        min_xyz = torch.stack(
            [
                env_o[:, 0] + (x_shift - wall_width * 0.5 + margin),
                env_o[:, 1] + (-wall_length * 0.5 + margin),
                env_o[:, 2] + (height_from_floor + margin),
            ],
            dim=-1,
        )
        max_xyz = torch.stack(
            [
                env_o[:, 0] + (x_shift + wall_width * 0.5 - margin),
                env_o[:, 1] + (wall_length * 0.5 - margin),
                env_o[:, 2] + (height_from_floor + wall_height - margin),
            ],
            dim=-1,
        )
        ...
        for i in range(self._task_cfg.max_num_goals):
            if i == 0:
                # First goal: Uniform inside ISS box AABB
                self._target_positions[env_ids, i, 0] = self._rng.sample_uniform_torch(
                    min_xyz[:, 0], max_xyz[:, 0], 1, ids=env_ids
                ).squeeze(-1)
                self._target_positions[env_ids, i, 1] = self._rng.sample_uniform_torch(
                    min_xyz[:, 1], max_xyz[:, 1], 1, ids=env_ids
                ).squeeze(-1)
                self._target_positions[env_ids, i, 2] = self._rng.sample_uniform_torch(
                    min_xyz[:, 2], max_xyz[:, 2], 1, ids=env_ids
                ).squeeze(-1)

                # First goal: Random orientation (yaw, pitch, roll)
                yaw_offset = self._rng.sample_uniform_torch(-math.pi, math.pi, 1, ids=env_ids)
                pitch_offset = self._rng.sample_uniform_torch(-math.pi / 2, math.pi / 2, 1, ids=env_ids)
                ...
                # If needed, randomize the next goals (position only)
                r = (
                ...
                cand_pos = torch.zeros_like(self._target_positions[env_ids, i])
                cand_pos[:, 0] = (
                    r.squeeze(-1) * torch.sin(phi.squeeze(-1)) * torch.cos(theta.squeeze(-1))
                    + self._target_positions[env_ids, i - 1, 0]
                )
                cand_pos[:, 1] = (
                    r.squeeze(-1) * torch.sin(phi.squeeze(-1)) * torch.sin(theta.squeeze(-1))
                    + self._target_positions[env_ids, i - 1, 1]
                )
                cand_pos[:, 2] = (
                    r.squeeze(-1) * torch.cos(phi.squeeze(-1)) + self._target_positions[env_ids, i - 1, 2]
                )

                # Clamp candidate goal to ISS box AABB
                self._target_positions[env_ids, i] = torch.max(torch.min(cand_pos, max_xyz), min_xyz)

                # Make all subsequent orientations equal to the first goal's orientation
                self._target_orientations[env_ids, i] = self._target_orientations[env_ids, 0]
